...
export const DailyJournal: React.FC<DailyJournalProps> = ({
  ...
  // Manejar actualizaciones de la cuenta
  useEffect(() => {
    ...
    const handleAccountUpdate = (data: any) => {
      ...
      // Si hay cambios en las posiciones, mostrar notificaciones
      if (data.position_changes?.length > 0) {
        data.position_changes.forEach((change: any) => {
          switch (change.type) {
                  ...
                  <div className="flex">
                    ...
                    <div className="ml-3">
                      <p className="text-sm text-blue-700">
                        New position opened: {change.position.symbol}
                      </p>
                    </div>
                  </div>
                </div>
                  ...
                  <div className="flex">
                    ...
                    <div className="ml-3">
                      <p className="text-sm text-gray-700">
                        Position closed: {change.position.symbol}
                      </p>
                    </div>
                  </div>
                </div>
            ...
            case 'position_updated':
              const profitChange = change.new_profit - change.old_profit;
              const isProfit = profitChange >= 0;
              toast.custom(
                <div className={`bg-${isProfit ? 'green' : 'red'}-50 border-l-4 border-${isProfit ? 'green' : 'red'}-400 p-4`}>
                  <div className="flex">
                    <div className="flex-shrink-0">
                      <svg className={`h-5 w-5 text-${isProfit ? 'green' : 'red'}-400`} viewBox="0 0 20 20" fill="currentColor">
                        <path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm1-11a1 1 0 10-2 0v3.586L7.707 9.293a1 1 0 00-1.414 1.414l3 3a1 1 0 001.414 0l3-3a1 1 0 00-1.414-1.414L11 10.586V7z" clipRule="evenodd" />
                      </svg>
                    </div>
                    <div className="ml-3">
                      <p className={`text-sm text-${isProfit ? 'green' : 'red'}-700`}>
                        Position updated: {change.position.symbol} ({profitChange.toFixed(2)})
                      </p>
                    </div>
                  </div>
                </div>
  ...
  const handleRefresh = async () => {
    if (!selectedAccount || !session?.user?.id) return;
      ...
      <div className="flex justify-between items-center mb-6">
        <div>
          ...
          <p className="text-sm text-gray-500">
            {isLoadingLocal ? 
              'Loading account data... (Connecting...)' : 
      ...
      {accountData && (
        <div className="grid grid-cols-4 gap-4 mb-6">
          <div className="p-4 bg-white rounded-lg shadow">
            ...
            <p className="text-2xl font-semibold">${accountData.balance?.toFixed(2)}</p>
          </div>
          <div className="p-4 bg-white rounded-lg shadow">
            <h3 className="text-sm font-medium text-gray-500">Equity</h3>
            <p className="text-2xl font-semibold">${accountData.equity?.toFixed(2)}</p>
          </div>
          <div className="p-4 bg-white rounded-lg shadow">
            <h3 className="text-sm font-medium text-gray-500">Floating P/L</h3>
            <p className={`text-2xl font-semibold ${accountData.floating_pl >= 0 ? 'text-green-600' : 'text-red-600'}`}>
              ${accountData.floating_pl?.toFixed(2)}
            </p>
          </div>
          <div className="p-4 bg-white rounded-lg shadow">
